...
class BsonVLADataset:
    ...
    """
    def __init__(self, bson_dir: str="data/ours/final_action1_only", sub_sample=1.0,
                 normalize_mode: str="mean_std", stats_file: str="1115pap_actions_stats/dataset_statistics.json",
                 actions: Optional[List[str]] = None, exclude_actions: Optional[List[str]] = None) -> None:
        """
    Initializes the BsonVLADataset.
        ...
        Args:
            ...
            actions (List[str], optional): Explicit list of action directories to include (e.g. ["action1", "action2"]). If None, include all starting with 'action'.
            exclude_actions (List[str], optional): List of action directories to skip.
        """
        ...
        include_set = set(actions) if actions else None
        exclude_set = set(exclude_actions) if exclude_actions else set()

        action_dirs = [d for d in os.listdir(bson_dir) if d.startswith('action') and os.path.isdir(os.path.join(bson_dir, d))]
        action_dirs.sort()

        used_action_dirs = []
        for action_item in action_dirs:
            if include_set and action_item not in include_set:
                continue
            if action_item in exclude_set:
                continue

            action_path = os.path.join(bson_dir, action_item)

            # Read instruction file (fallback names supported)
            instruction_candidates = ["instruction1.txt", "instruction.txt", "action.txt"]
            action_text = None
            for inst_name in instruction_candidates:
                action_txt_path = os.path.join(action_path, inst_name)
                if os.path.exists(action_txt_path):
                    try:
                        with open(action_txt_path, 'r') as f:
                            action_text = f.read().strip()
                    except Exception as e:
                        print(f"Warning: Failed reading {action_txt_path}: {e}")
                    break
            if action_text is None:
                print(f"Warning: Missing instruction file in {action_path}, skipping this action directory.")
                continue

            # Iterate episode subfolders
            for episode_item in os.listdir(action_path):
                episode_path = os.path.join(action_path, episode_item)
                if not (os.path.isdir(episode_path) and episode_item.startswith('episode')):
                    continue

                main_bson = os.path.join(episode_path, "episode_0.bson")
                xhand_bson = os.path.join(episode_path, "xhand_control_data.bson")

                if not (os.path.exists(main_bson) and os.path.exists(xhand_bson)):
                ...
                used_action_dirs.append(action_item)

        if include_set:
            missing = include_set - set(used_action_dirs)
            if missing:
                print(f"Note: These requested action dirs were not found or had no valid episodes: {sorted(missing)}")
        
        # Sort by episode path for consistency
        self.episode_infos.sort(key=lambda x: x.path)
        unique_actions_used = sorted({Path(e.path).parent.name for e in self.episode_infos})
        print(f"Found {len(self.episode_infos)} valid episode directories from {len(unique_actions_used)} action dirs: {unique_actions_used[:10]}{'...' if len(unique_actions_used)>10 else ''}")
    ...
    def _extract_data_from_episode(self, episode_info: EpisodeInfo) -> Optional[Dict]:
        ...
        for i in range(frame_num):
            # Extract raw hand observations (assumed degrees) and convert to radians
            left_hand_obs_raw = xhand_data['frames'][i]["observation"]["left_hand"]
            right_hand_obs_raw = xhand_data['frames'][i]["observation"]["right_hand"]
            left_hand_obs_rad = np.deg2rad(np.array(left_hand_obs_raw, dtype=np.float32))
            right_hand_obs_rad = np.deg2rad(np.array(right_hand_obs_raw, dtype=np.float32))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--bson_dir', type=str, default='data/ours/final', help='Root directory containing action*/episode* data')
    parser.add_argument('--sub_sample', type=float, default=1.0, help='Fraction of episodes to keep or (if >0.5) keep tail part')
    parser.add_argument('--actions', type=str, help='Comma-separated action directories to include (overrides automatic discovery)')
    parser.add_argument('--actions_file', type=str, help='JSON file with key "actions" listing action directories to include')
    parser.add_argument('--exclude_actions', type=str, help='Comma-separated action directories to exclude')
    
    args = parser.parse_args()
    
    # --- Dataset Initialization ---
    # Parse action include/exclude lists
    actions_list = None
    if args.actions_file:
        try:
            with open(args.actions_file, 'r') as f:
                data_json = json.load(f)
            if isinstance(data_json, dict) and 'actions' in data_json and isinstance(data_json['actions'], list):
                actions_list = [str(a).strip() for a in data_json['actions']]
            else:
                print(f"Warning: actions_file {args.actions_file} does not contain key 'actions' as a list; ignoring.")
        except Exception as e:
            print(f"Warning: Failed to read actions_file {args.actions_file}: {e}")
    elif args.actions:
        actions_list = [a.strip() for a in args.actions.split(',') if a.strip()]

    exclude_list = [a.strip() for a in args.exclude_actions.split(',')] if args.exclude_actions else None

    ds = BsonVLADataset(bson_dir=args.bson_dir,
                        sub_sample=args.sub_sample,
                        normalize_mode=args.normalize,
                        stats_file=args.stats_file,
                        actions=actions_list,
                        exclude_actions=exclude_list)
