...
class VLAConsumerDataset(Dataset):
    ...
    def __init__(
        ...
        self.last_meta = None
        # Optional wrapped dataset (e.g., ScoringDataset). Can be assigned externally.
        self.dataset = getattr(self, 'dataset', None)
    ...
    def __len__(self) -> int:
        # If a wrapped dataset (e.g., ScoringDataset) is provided, respect its length.
    ...
    def __getitem__(self, index):
        ...
        while True:
            ...
            try:
                if self.use_hdf5:
                    # Prefer wrapped dataset if provided (e.g., ScoringDataset)
                    if getattr(self, 'dataset', None) is not None:
                ...
                # Propagate optional fields from underlying dataset
                if self.use_hdf5 and 'is_expert' in res:
                    # Ensure tensor dtype float32
                    is_exp = res['is_expert']
                    if isinstance(is_exp, np.ndarray):
                        is_exp = torch.from_numpy(is_exp)
                    elif not isinstance(is_exp, torch.Tensor):
                        is_exp = torch.tensor(is_exp)
                    data_dict['is_expert'] = is_exp.to(dtype=torch.float32)
                if self.use_hdf5 and 'logpi' in res:
                    lp = res['logpi']
                    if isinstance(lp, np.ndarray):
                        lp = torch.from_numpy(lp)
                    elif not isinstance(lp, torch.Tensor):
                        lp = torch.tensor(lp)
                    data_dict['logpi'] = lp.to(dtype=torch.float32)
                ...
                # Print the error info occasionally and avoid infinite hang
                retries += 1
                if retries <= 5 or retries % 50 == 0:
                    try:
                        ds_name = data_dict.get('dataset_name') if data_dict else None
                    except Exception:
                        ds_name = None
                    print(f"[WARN] Exception in VLAConsumerDataset.__getitem__ (retry {retries})"
                          f" dataset={ds_name}, idx={index}: {e}")
                    import traceback as _tb
                    _tb.print_exc()
                if retries > 50:
                    # Surface the error instead of hanging forever
                    raise
                # Try increasing the index
...
class DataCollatorForVLAConsumerDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            ...
            # Optional fields
            if 'is_expert' in instance:
                batch.setdefault('is_expert', []).append(
                    instance['is_expert'] if isinstance(instance['is_expert'], torch.Tensor)
                    else torch.tensor(instance['is_expert'], dtype=torch.float32)
                )
            if 'logpi' in instance:
                lp = instance['logpi'] if isinstance(instance['logpi'], torch.Tensor) else torch.tensor(instance['logpi'], dtype=torch.float32)
                batch.setdefault('logpi', []).append(lp)
        ...
        # Stack optional fields if present
        if 'is_expert' in batch:
            batch['is_expert'] = torch.stack([x.to(dtype=torch.float32).view(1) if x.ndim == 0 else x.to(dtype=torch.float32).view(-1) for x in batch['is_expert']], dim=0).view(-1)
        if 'logpi' in batch:
            # Expect shape [B, 1]
            batch['logpi'] = torch.stack([x.view(-1) if x.ndim > 0 else x.view(1) for x in batch['logpi']], dim=0)
